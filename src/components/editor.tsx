...
interface EditorProps {
  onSubmitted: ({ image, body }: EditorValue) => void;
  onCancel?: () => void;
  placeholder?: string;
    ...
    <div className=" flex flex-col">
      ...
      >
        ...
        <div className=" flex px-2 pb-2 z-[5]">
          <Hint hint={isToolbarVisible ? "Hide formatting" : "Show formatting"}>
            <Button
              disabled={disabled}
              size="iconSm"
              variant="ghost"
            ...
            >
              <PiTextAa
                className={cn(
                  "size-4",
                  isToolbarVisible ? "text-blue-600" : ""
                )}
              />
            </Button>
          </Hint>
          ...
          {variant === "create" && (
            <Button
              className={cn(
                " ml-auto",
                isEmptyText
                  ? "bg-white hover:bg-white text-mutable-foreground cursor-not-allowed"
                  : "bg-[#007a5a] hover:bg-[#007a5a] text-white"
              )}
              disabled={disabled || isEmptyText}
              size="iconSm"
              onClick={() => {
                onSubmitted({
                  body: JSON.stringify(quillRef.current?.getContents()),
      ...
      {variant === "create" && (
        <div
          className={cn(
            " p-2 text-[10px] text-muted-foreground opacity-0 flex justify-end",
            isEmptyText && "opacity-100"
          )}
        >
          <strong className=" pr-1">Shift + Return</strong>
